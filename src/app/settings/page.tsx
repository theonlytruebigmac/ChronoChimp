...
import {
  ...
  DialogContent,
  DialogHeader,
  DialogTitle as TwoFaDialogTitle,
  DialogDescription as TwoFaDialogDescription,
  DialogFooter as TwoFaDialogFooter,
...
export default function SettingsPage() {
  ...
  const [profileData, setProfileData] =
    useState<Partial<UserProfile>>(initialProfileState);
  ...
  const [newApiKeyExpiresInDays, setNewApiKeyExpiresInDays] =
    useState<string>("");
    ...
    if (userProfile) {
      setProfileData({
        ...
        emailNotificationsEnabled:
          userProfile.emailNotificationsEnabled !== undefined
            ? userProfile.emailNotificationsEnabled
            : true,
        inAppNotificationsEnabled:
          userProfile.inAppNotificationsEnabled !== undefined
            ? userProfile.inAppNotificationsEnabled
            : true,
        isTwoFactorEnabled:
          userProfile.isTwoFactorEnabled !== undefined
            ? userProfile.isTwoFactorEnabled
            : false,
      });
      setProfilePreview(
        userProfile.avatarUrl || "https://placehold.co/100x100.png"
      );
    ...
    onSuccess: (updatedProfile) => {
      ...
      if (updatedProfile) {
        ...
        if (updatedProfile.avatarUrl)
          setProfilePreview(updatedProfile.avatarUrl);
        // Update SMTP state if they were part of this update
        if (updatedProfile.smtpHost !== undefined)
          setSmtpHost(updatedProfile.smtpHost || "");
        if (updatedProfile.smtpPort !== undefined)
          setSmtpPort(
            updatedProfile.smtpPort ? String(updatedProfile.smtpPort) : ""
          );
        if (updatedProfile.smtpEncryption !== undefined)
          setSmtpEncryption(updatedProfile.smtpEncryption || "starttls");
        if (updatedProfile.smtpUsername !== undefined)
          setSmtpUsername(updatedProfile.smtpUsername || "");
        if (updatedProfile.smtpSendFrom !== undefined)
          setSmtpSendFrom(updatedProfile.smtpSendFrom || "");
      }
      toast({
  ...
  const handleSaveChanges = () => {
    if (!profileData.name || !profileData.email) {
      toast({
        ...
        description: "Name and Email are required for profile.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleGenerateNewApiKey = () => {
    ...
    const expiresIn = newApiKeyExpiresInDays
      ? parseInt(newApiKeyExpiresInDays, 10)
      : null;
    if (
      newApiKeyExpiresInDays &&
      (isNaN(expiresIn as number) || (expiresIn as number) <= 0)
    ) {
      toast({
        title: "Invalid Expiration",
        description: "Expiration must be a positive number of days.",
        variant: "destructive",
      });
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (successful) {
          toast({
            title: "Copied!",
            description: "API Key copied using fallback method.",
          });
          resolve();
        } else {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (successful) {
          toast({
            title: "Copied!",
            description: "API Key copied using range selection.",
          });
          resolve();
        } else {
  ...
  const showManualCopyInstructions = () => {
    toast({
      ...
      description:
        "Please click the API key field to select it, then use your device's copy function.",
      variant: "destructive",
      duration: 5000,
    });
  };
  ...
  const handleTestSmtp = async () => {
    ...
    if (!smtpHost || !smtpSendFrom) {
      toast({
        title: "Missing Fields",
        description: "SMTP Host and Send From Email are required for testing.",
        variant: "destructive",
      });
      setIsTestingSmtp(false);
      return;
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `${result.message} (Host: ${smtpHost}, Port: ${smtpPort}, From: ${smtpSendFrom})`,
        });
      } else {
        toast({
  ...
  const handleNotificationToggle = (
    type: "emailNotificationsEnabled" | "inAppNotificationsEnabled",
    enabled: boolean
  ) => {
  ...
  const initiate2FASetup = async () => {
    try {
      const response = await fetch("/api/me/2fa/setup-initiate", {
        method: "POST",
      });
      if (!response.ok) {
  ...
  const handleVerifyAndEnable2FA = async () => {
    ...
    try {
      ...
      const encryptedSecret = encryptedSecretKey
        ? sessionStorage.getItem(encryptedSecretKey)
        : null;
      ...
      toast({
        title: "2FA Enabled",
        description:
          "Two-Factor Authentication has been successfully enabled. Generating backup codes...",
      });
      ...
      toast({
        title: "Invalid OTP",
        description: err.message || "Please enter a valid 6-digit OTP.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleDisable2FA = () => {
    ...
    toast({
      ...
      description:
        'Click "Save Profile" at the top to persist this change and clear your 2FA secret.',
    });
  };
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your profile information, security settings, API keys, and
            interface theme.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-0 pt-0">
            ...
            <AccordionItem value="api-keys">
              ...
              <AccordionContent className="pt-4 space-y-4">
                  ...
                  >
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Enter a descriptive name and optional expiration for
                          your new API key.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="apiKeyExpiresInDays">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setNewApiKeyExpiresInDays(e.target.value)
                          }
                ...
                {generatedApiKey && (
                  <AlertDialog
                    defaultOpen
                    onOpenChange={() => setGeneratedApiKey(null)}
                  >
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                        ...
                        <AlertDialogDescription>
                          Please copy your new API key. You won't be able to see
                          it again.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <div className="space-y-4 mt-2 p-3 bg-muted rounded-md border">
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                copyToClipboard(generatedApiKey.fullKey)
                              }
                            >
                          ...
                          <div className="text-xs text-muted-foreground bg-muted/50 p-2 rounded">
                            ...
                            <ol className="list-decimal list-inside space-y-1">
                              <li>
                                Click the text area above to select all text
                              </li>
                              <li>
                        ...
                        {generatedApiKey.expiresAt && (
                          <p className="text-xs text-muted-foreground">
                            Expires: {formatDateSafe(generatedApiKey.expiresAt)}
                          </p>
                        )}
                      </div>
                      <AlertDialogFooter>
                              ...
                              <AlertDialogTrigger asChild>
                                <Button
                                  ...
                                  disabled={
                                    ...
                                    revokeApiKeyMutation.variables === key.id
                                  }
                                >
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    permanently revoke the API key "{key.name}".
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                                  ...
                                  >
                                    {revokeApiKeyMutation.isPending &&
                                    revokeApiKeyMutation.variables ===
                                      key.id ? (
                                      <>
                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />{" "}
                                        Revoking...
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage notification preferences and SMTP configuration.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-0 pt-0">
          <Accordion type="multiple" className="w-full">
            <AccordionItem value="custom-fields">
              <AccordionTrigger className="text-lg font-medium hover:no-underline opacity-50 pointer-events-none">
                <div className="flex items-center gap-2">
            ...
            <AccordionItem value="task-tags">
              <AccordionTrigger className="text-lg font-medium hover:no-underline opacity-50 pointer-events-none">
                <div className="flex items-center gap-2">
                  <Tag className="h-4 w-4 text-muted-foreground" />
                  Task Tags (Coming Soon)
                </div>
              </AccordionTrigger>
            </AccordionItem>
          </Accordion>
          <Accordion type="multiple" className="w-full">
              ...
              <AccordionContent className="pt-4 space-y-4">
                <div className="flex items-center justify-between rounded-lg border p-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Enable to receive email updates for task activities.
                    </p>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between rounded-lg border p-4 mt-4">
                  <div>
                    <Label htmlFor="inAppNotifications" className="font-medium">
                      In-App Notifications for Due Dates
                    </Label>
                    <p className="text-sm text-muted-foreground">
                      Enable to receive in-app reminders for task due dates.
                    </p>
                  </div>
                  <Switch
            ...
            <AccordionItem value="smtp-settings">
              ...
              <AccordionContent className="pt-4 space-y-4">
                <p className="text-sm text-muted-foreground">
                  Configure your custom SMTP server for sending email
                  notifications.
                </p>
                <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2 mt-4">
                  <Button
                    ...
                    disabled={isSavingSmtp || profileUpdateMutation.isPending}
                  >
                    {isSavingSmtp ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving
                      ...
                      <>
                        <Server className="mr-2 h-4 w-4" /> Save SMTP Settings
                      </>
                    )}
                  </Button>
                  <Button
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <TwoFaDialogTitle className="flex items-center">
              <QrCode className="mr-2 h-5 w-5" />
              Setup Two-Factor Authentication
            </TwoFaDialogTitle>
            <TwoFaDialogDescription>
              Scan the QR code with your authenticator app (e.g., Google
              Authenticator, Authy). Then, enter the 6-digit code from your app
              to verify. After verifying your 2FA setup, you'll be able to
            ...
            {qrCodeDataUrl ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  <Label htmlFor="twoFactorSecretForDialog">
          ...
          <TwoFaDialogFooter>
            <Button
              ...
              onClick={() => {
                ...
                if (
                  profileData.isTwoFactorEnabled !== currentProfile2FAState ||
                  !currentProfile2FAState
                ) {
                  refetchProfile();
                }
              }}
            ...
            >
              ...
              {profileUpdateMutation.isPending && !isVerifyingOtp && (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  Saving Profile...
                </>
              )}
              {!isVerifyingOtp && !profileUpdateMutation.isPending && (
                <>
